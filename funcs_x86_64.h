...
void ScanWrite64IndexUnrollLoop(char* memarea, size_t size, size_t repeats)
{
    asm volatile(
        ...
        "xor    %%rcx, %%rcx \n"        // rcx = reset index
        "2: \n" // start of write loop
        "movnti    %%rax, 0*8(%[memarea],%%rcx) \n"
        "movnti    %%rax, 1*8(%[memarea],%%rcx) \n"
        "movnti    %%rax, 2*8(%[memarea],%%rcx) \n"
        "movnti    %%rax, 3*8(%[memarea],%%rcx) \n"
        "movnti    %%rax, 4*8(%[memarea],%%rcx) \n"
        "movnti    %%rax, 5*8(%[memarea],%%rcx) \n"
        "movnti    %%rax, 6*8(%[memarea],%%rcx) \n"
        "movnti    %%rax, 7*8(%[memarea],%%rcx) \n"
        "movnti    %%rax, 8*8(%[memarea],%%rcx) \n"
        "movnti    %%rax, 9*8(%[memarea],%%rcx) \n"
        "movnti    %%rax, 10*8(%[memarea],%%rcx) \n"
        "movnti    %%rax, 11*8(%[memarea],%%rcx) \n"
        "movnti    %%rax, 12*8(%[memarea],%%rcx) \n"
